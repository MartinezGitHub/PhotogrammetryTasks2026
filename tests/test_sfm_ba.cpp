...
#define NIMGS_LIMIT                           21 // сколько фотографий обрабатывать (можно выставить меньше чтобы ускорить экспериментирование, или в случае если весь датасет не выравнивается)
...
// учитывать стоит, с k1 k2 меньше ошибка репроекци
...
// без фильтрации рез хуже, больше шума и улетевших точек
...
// потому что фокальная длина здесь измеряется в пикселях, если мы уменьшили картинку в downscale раз, размер пиксельной сетки тоже уменьшился в downscale раз.
// Все пиксельные координаты надо делить на downscale. 
...
public:
    ReprojectionError(double x, double y, int width, int height)
        : observed_x(x)
        , observed_y(y)
        , width(width)
        , height(height)
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        T point_centered[3] = {point_global[0] - translation[0], point_global[1] - translation[1], point_global[2] - translation[2]};
        ...
        T point_local[3];
        ceres::AngleAxisRotatePoint(rotation, point_centered, point_local);

        // Проецируем точку на фокальную плоскость матрицы (т.е. плоскость Z=фокальная длина)
        T x = point_local[0] / point_local[2];
        T y = point_local[1] / point_local[2];
        ...
        x += camera_intrinsics[3] + (T) width * (T) 0.5;
        y += camera_intrinsics[4] + (T) height * (T) 0.5;
           ...
           bool verbose)
{
    ...
    double camera_intrinsics[5] = {calib.k1_, calib.k2_, calib.f_, calib.cx_, calib.cy_};
    std::cout << "Before BA ";
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            ceres::CostFunction* keypoint_reprojection_residual = new ceres::AutoDiffCostFunction<ReprojectionError,
                    ...
                    (new ReprojectionError(px[0], px[1], calib.width(), calib.height()));
...
//http://ceres-solver.org/nnls_solving.html
    ...
    calib.cx_ = camera_intrinsics[3];
    calib.cy_ = camera_intrinsics[4];
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
                ...
                double min_angle_cos = 1.0;
                for (size_t cj = 0; cj < track.img_kpt_pairs.size(); ++cj) {
                    int other_camera_id = track.img_kpt_pairs[cj].first;
                    if (other_camera_id == camera_id) {
                        continue;
                    }

                    matrix3d other_R;
                    vector3d other_camera_origin;
                    phg::decomposeUndistortedPMatrix(other_R, other_camera_origin, cameras[other_camera_id]);

                    vector3d ray0 = track_point - camera_origin;
                    vector3d ray1 = track_point - other_camera_origin;
                    double ray0_norm = cv::norm(ray0);
                    double ray1_norm = cv::norm(ray1);
                    if (ray0_norm == 0.0 || ray1_norm == 0.0) {
                        should_be_disabled = true;
                        continue;
                    }
                    ...
                    ray0 /= ray0_norm;
                    ray1 /= ray1_norm;
                    double angle_cos = ray0.dot(ray1);
                    min_angle_cos = std::min(min_angle_cos, std::min(1.0, std::max(-1.0, angle_cos)));
                }

                if (min_angle_cos > std::cos(2.5 * CV_PI / 180.0)) {
                    should_be_disabled = true;
                }
            }
