...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            A.row(2 * i) << 0.0, 0.0, 0.0, 0.0, -w * X, -w * Y, -w * Z, -w * W, y * X, y * Y, y * Z, y * W;
            A.row(2 * i + 1) << w * X, w * Y, w * Z, w * W, 0.0, 0.0, 0.0, 0.0, -x * X, -x * Y, -x * Z, -x * W;
        }
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                ...
                p /= p[2];
                cv::Vec3d pxh = calib.project(p);
                if (pxh[2] == 0) {
                    continue;
                }
                cv::Vec2d px(pxh[0] / pxh[2], pxh[1] / pxh[2]);
        ...
        std::vector<cv::Vec3d> inliers_X;
        std::vector<cv::Vec3d> inliers_x;
        for (int i = 0; i < n_points; ++i) {
            vector4d Xh(X[i][0], X[i][1], X[i][2], 1.0);
            vector3d p = best_P * Xh;
            if (p[2] == 0) {
                continue;
            }
            p /= p[2];
            ...
            cv::Vec2d px(pxh[0] / pxh[2], pxh[1] / pxh[2]);
            if (cv::norm(px - x[i]) < threshold_px) {
                inliers_X.push_back(X[i]);
                inliers_x.push_back(calib.unproject(x[i]));
            }
        }

        if ((int) inliers_X.size() >= n_samples) {
            best_P = estimateCameraMatrixDLT(inliers_X.data(), inliers_x.data(), inliers_X.size());
        }
