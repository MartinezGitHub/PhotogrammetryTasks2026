...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
    ...
    {
        if (m.empty()) {
            throw std::runtime_error("getNormalizeTransform: empty points");
        }
        ...
        double rms = 0.0;
        for (const cv::Vec2d &pt : m) {
            cv::Vec2d d = pt - center;
            rms += d.dot(d);
        }
        rms = std::sqrt(rms / m.size());

        if (rms == 0.0) {
            throw std::runtime_error("getNormalizeTransform: zero rms");
        }

        double scale = std::sqrt(2.0) / rms;
        return cv::Matx33d(scale, 0.0, -scale * center[0], 0.0, scale, -scale * center[1], 0.0, 0.0, 1.0);
    }
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
        ...
        const int n_trials = n_matches > 10000 ? 100000 : 2000;
